...
#[cfg(send_sync)]
pub type BoxSubmittedWorkDoneResultCallback =
    Box<dyn FnOnce(Result<(), crate::SubmittedWorkDoneError>) + Send + 'static>;
#[cfg(not(send_sync))]
pub type BoxSubmittedWorkDoneResultCallback =
    Box<dyn FnOnce(Result<(), crate::SubmittedWorkDoneError>) + 'static>;
#[cfg(send_sync)]
...
pub trait QueueInterface: CommonTraits {
    ...
    fn on_submitted_work_done_result(&self, callback: BoxSubmittedWorkDoneResultCallback) {
        self.on_submitted_work_done(Box::new(move || callback(Ok(()))));
    }
