...
// Cargo caps warnings for registry dependencies but not local path patches.
// Preserve that behavior for the upstream pure-WebGPU cfg so workspace-wide
// `RUSTFLAGS=-D warnings` does not promote wgpu's existing cfg-only warnings.
#![cfg_attr(webgpu, allow(dead_code, unused_imports, unused_macros))]
