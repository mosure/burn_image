...
/// Error returned when browser WebGPU rejects its queue-completion promise.
///
/// Native backends have no equivalent rejection channel and report successful completion once
/// their existing submitted-work callback runs.
#[derive(Clone, PartialEq, Eq, Debug)]
pub struct SubmittedWorkDoneError {
    reason: String,
}
static_assertions::assert_impl_all!(SubmittedWorkDoneError: Send, Sync);

impl SubmittedWorkDoneError {
    #[cfg(webgpu)]
    pub(crate) fn webgpu_promise_rejected(reason: String) -> Self {
        Self { reason }
    }
}

impl fmt::Display for SubmittedWorkDoneError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "WebGPU queue completion promise was rejected: {}",
            self.reason
        )
    }
}

impl core::error::Error for SubmittedWorkDoneError {}
...
impl Queue {
    ...
    /// Registers a callback that reports whether the previous submissions completed.
    ///
    /// This is equivalent to [`Queue::on_submitted_work_done`], except browser WebGPU rejection
    /// of `GPUQueue.onSubmittedWorkDone()` is preserved as [`SubmittedWorkDoneError`]. Native
    /// backends pass `Ok(())` when their existing submitted-work callback runs.
    pub fn on_submitted_work_done_result(
        &self,
        callback: impl FnOnce(Result<(), SubmittedWorkDoneError>) + Send + 'static,
    ) {
        self.inner.on_submitted_work_done_result(Box::new(callback));
    }
