...
impl dispatch::QueueInterface for WebQueue {
    fn write_buffer(
        ...
        for chunk in super::webgpu_write::write_buffer_chunks(offset, data.len()) {
            let data = &data[chunk.data_range];
            self.inner
                .write_buffer_with_f64_and_u8_slice_and_f64_and_f64(
                    &buffer.inner,
                    chunk.buffer_offset as f64,
                    data,
                    0f64,
                    data.len() as f64,
                )
    ...
    fn write_texture(
        ...
        let format = texture.texture.descriptor.format;
        let (block_width_texels, block_height_texels) = format.block_dimensions();
        let chunks = format
            .block_copy_size(Some(texture.aspect))
            .and_then(|block_size_bytes| {
                super::webgpu_write::write_texture_chunks(
                    super::webgpu_write::WriteTextureCopy {
                        data_offset: data_layout.offset,
        ...
        if let Some(chunks) = chunks {
            for chunk in chunks {
                let chunk_texture = crate::TexelCopyTextureInfo {
                ...
                mapped_data_layout.set_offset(0f64);

                self.inner
                    .write_texture_with_u8_slice_and_gpu_extent_3d_dict(
                        &map_texture_copy_view(chunk_texture),
                        &data[chunk.data_range],
                        &mapped_data_layout,
                        &map_extent_3d(crate::Extent3d {
                            width: size.width,
                            height: chunk.height,
    ...
    fn on_submitted_work_done(&self, callback: dispatch::BoxSubmittedWorkDoneCallback) {
        self.on_submitted_work_done_result(Box::new(move |result| {
    ...
    fn on_submitted_work_done_result(
        &self,
        callback: dispatch::BoxSubmittedWorkDoneResultCallback,
    ) {
        ...
        wasm_bindgen_futures::spawn_local(async move {
            match wasm_bindgen_futures::JsFuture::from(promise).await {
                ...
                Err(error) => {
                    ...
                    callback(Err(crate::SubmittedWorkDoneError::webgpu_promise_rejected(
                        reason,
                    )));
                }
            }
