...
fn register_features(
    ...
    // This helper is reached only for a device requested by CubeCL's Vulkan path, which enabled
    // the complete raw feature chain represented by `extended_feat` during device creation.
