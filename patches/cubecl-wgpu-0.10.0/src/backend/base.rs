...
use wgpu::{
    ...
    ComputePipeline, Device, Features, PipelineLayoutDescriptor, Queue, ShaderModule,
...
impl WgpuServer {
    ...
    /// Returns `None` if the cache isn't enabled, `Some(Ok((pipeline, entrypoint)))` if a cache
    /// entry was found, and `Some(Err(cache_key))` if the cache is enabled but doesn't contain
    /// this kernel. The exact entrypoint lets the caller retain actionable validation scope
    /// context on WebGPU.
    #[allow(
    ...
    pub fn load_cached_pipeline(
        ...
        mode: ExecutionMode,
    ) -> Result<Option<Result<(Arc<ComputePipeline>, String), (u64, StableHash)>>, CompilationError>
    {
        ...
        let res = if let Some(cache) = &self.spirv_cache {
            ...
            if let Some(entry) = cache.get(&key) {
                ...
                Ok(Some(Ok((pipeline, entry.entrypoint_name.clone()))))
            } else {
                Ok(Some(Err(key)))
            }
        } else {
    ...
    pub fn create_module(
        ...
        match repr {
            ...
            _ => {
                ...
                // Native callers preserve the existing synchronous shader-compilation error.
                // WebGPU can't block here, so `WgpuServer::pipeline` owns outer
                // Validation/OOM/Internal scopes for module and pipeline creation; their pop
                // futures are retained on the launch stream until explicit sync.
                ...
                #[cfg(not(target_family = "wasm"))]
                {
                    // `pop()` detaches from the LIFO stack immediately; only the result is async.
...
pub fn register_features(
    ...
    if register_vulkan_features(adapter, device_features, props, comp_options, memory_config) {
        return;
    }
    if register_metal_features(adapter, device_features, props, comp_options, memory_config) {
        return;
    }
    wgsl::register_wgsl_features(device_features, props, comp_options);
}
