...
pub fn register_metal_features(
    ...
    unsafe {
        if let Some(adapter) = adapter.as_hal::<hal::api::Metal>() {
            register_features(&adapter, props, device_features, comp_options);
        }
    }
}
...
fn register_features(
    ...
    register_types(props, features);
    if features.contains(Features::EXPERIMENTAL_COOPERATIVE_MATRIX) {
        register_cmma(props);
    }
    ...
    if features.contains(Features::SUBGROUP_BARRIER) {
        props.features.plane.insert(Plane::Sync);
    }
}

fn register_types(props: &mut DeviceProperties, device_features: Features) {
    use cubecl_core::ir::{ElemType, FloatKind, IntKind, StorageType};

    props.register_address_type(AddressType::U32);
    if device_features.contains(Features::SHADER_INT64) {
    ...
    let atomic_types = [ElemType::Int(IntKind::I32), ElemType::UInt(UIntKind::U32)];
    ...
    if device_features.contains(Features::SHADER_F16) {
    ...
    if device_features
        .intersects(Features::SHADER_INT64_ATOMIC_MIN_MAX | Features::SHADER_INT64_ATOMIC_ALL_OPS)
    {
        let usage = if device_features.contains(Features::SHADER_INT64_ATOMIC_ALL_OPS) {
            AtomicUsage::all()
        } else {
            AtomicUsage::MinMax.into()
        };
        props.register_atomic_type_usage(
    ...
    if device_features.contains(Features::SHADER_FLOAT32_ATOMIC) {
        props.register_atomic_type_usage(
